...
(startIndex) => {
  ...
  function emit(node, depth, long, itemName) {
    ...
    // Безымянная отметка внутри записи — это выбор записи для массовых действий:
    // общее соглашение списков (почта, таблицы, корзина). Без подписи модель
    // предпочла соседнюю кнопку с понятной подписью — «Не помечено» у звёздочки
    // прочитала как «не отмечено» и пометила два письма избранными (прогон 29).
    if (insideItem && !name) {
      const r = (node.getAttribute('role') || '').toLowerCase();
      const t = node.tagName.toLowerCase() === 'input' ? node.type : '';
      if (r === 'checkbox' || r === 'radio' || t === 'checkbox' || t === 'radio') {
        name = 'выбор записи';
        blanked = false;
      }
    }
