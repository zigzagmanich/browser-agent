...
- [Исследование: что ломалось и как это находилось](#исследование-что-ломалось-и-как-это-находилось)
...
Полный дневник исследования — [docs/RESEARCH.md](docs/RESEARCH.md): 45 найденных дефектов с цифрами до и после, разбор 25 живых прогонов, эксперименты и решения, которые не сработали.
...
playwright install chromium         # нужен, только если в системе нет Google Chrome
...
Под-агенты специализированы по способу работы с данными, а не по предметной области. `classify` один и тот же для «это спам?», «вакансия подходит под резюме?» и «это бургер, а не соус?» — критерий формулирует оркестратор в моменте. Иначе получились бы те самые запрещённые заготовки.
...
## Исследование: что ломалось и как это находилось

Метод: сначала смотреть глазами модели, потом чинить. Для этого сделан стенд `probe.py` — браузер без LLM, где видно снапшот, сырые узлы, подсветку проиндексированного прямо на странице и ответ на вопрос «почему этого нет в индексе» (`find CSS`, `find text=Сортировка` с «носителем курсора»). Перед каждой правкой индексатора — замер до и после, после каждой — проверка, что прошлые сайты не сломались. Правки шли по Gmail, hh.ru, маркетплейсу, Яндекс Почте, Лавке, Авито, Островку, GitHub.

### Восприятие — избранное

| что было | как нашлось | что сделано |
|---|---|---|
| строка письма разваливалась на 6 рефов, из них 4 пустых `<td>` | снапшот Gmail | «составная запись» — строка списка или карточка индексируется одним рефом, внутри остаются только органы управления |
| в индекс лезли «Опыт 1-3 года», адреса, рейтинги — некликабельные подписи | hh.ru: 622 элемента | `cursor` наследуется: носитель курсора — тот, у кого значение отличается от родителя. 622 → 417 элементов, снапшот 4 113 → 2 411 токенов |
| модалка не индексировалась вовсе, «на экране 0» | клик по «−» в корзине | окно рендерилось в обёртке нулевого размера. Видимость стала тройной: скрыт / без бокса (обходить потомков) / видим |
| агент удалял письма по одному: чекбоксов выбора не было в снапшоте | прогон 7, потом `find` в стенде | настоящий `<input type=checkbox>` прозрачный и лежит поверх нарисованного. Прозрачный элемент формы остаётся в индексе, пока принимает клики. В следующем прогоне — 4 письма отмечены и удалены одним нажатием |
| превью писем наполовину из пробелов нулевой ширины | снапшот Яндекс Почты | невидимые символы Юникода вычищаются: строка письма 160 → 52 символа, на освободившемся месте — настоящий текст |
| календарь дат — одна кнопка «ПН ВТ … 1 2 3 … 30» | Островок, `find text=25` | 7+ одинаковых кликабельных ячеек внутри кнопки — это сетка, у каждой ячейки свой реф. Дни за краем прокручиваемого календаря выводятся первыми, отдельным разделом |
| кнопки «Сортировка» нет в индексе | Авито, строка «носитель курсора: `<a>` НЕ в индексе» | `<a>` без `href` с обработчиком в скрипте отбрасывался раньше проверки курсора. Первая гипотеза (спрятанный контрол) была неверной — стенд её опроверг за один запуск |
| резюме не читалось: «фрагмент — JavaScript-код Sentry» | прогон 17, лог | скрытый служебный фрейм отдавал 50 тысяч символов кода. Скрытые и крошечные фреймы не читаются и не индексируются: резюме 107 689 → 5 288 символов |
| название товара трижды в карточке | Лавка | повтор фразы из 3+ слов выбрасывается: −6% снапшота, в строках ниже экрана стала видна цена |

### Действия

| что было | как нашлось | что сделано |
|---|---|---|
| агент не мог завершить задачу и крутился до лимита в 60 шагов | чтение кода перед первым прогоном | общий `except Exception` превращал штатный `finish` в «ОШИБКА Finished» |
| одно подтверждение — два нажатия «Удалить» | эксперимент: человек думает дольше таймаута клика | клик падал по таймауту, запасной программный клик нажимал второй раз. Счётчик открытых системных окон запрещает запасной клик, если окно уже открылось |
| «3500» в фильтр цены не вставало ни с какой попытки | прогоны 22–23, стенд дал «0100000» | поле проверяет каждую клавишу: «3», «35», «350» меньше нижней границы → сайт возвращает максимум. После посимвольного ввода — проверка значения, при провале — вставка целиком. Подтверждено на живом сайте |
| Ctrl+C закрывал браузер и терял логины | эксперимент: SIGINT группе процессов, 4 варианта | виноват сам Playwright: при `handle_sigint=True` драйвер на Ctrl+C закрывает браузер. Флаг выключен, драйвер вынесен в свою группу процессов |

### Сверка с известными провалами других агентов

Не дожидаясь, пока каждая ошибка всплывёт в прогоне, агент сверен с разборами трасс веб-агентов ([Invariant Labs](https://invariantlabs.ai/blog/what-we-learned-from-analyzing-web-agents), [FutureAGI](https://futureagi.com/blog/evaluating-browser-use-agents-2026/), [Agent-E](https://arxiv.org/abs/2407.13032), трекер browser-use). Большая часть уже была закрыта находками выше: текст дописывается к старому (у Invariant агент вбил запрос 20 раз), выпадающие списки, модалки и баннеры, выдуманные данные в формах, игнор ограничений задачи. Три дыры закрыты заранее: меню только по наведению (`hover`), нативное поле даты с форматом не той локали (агент вписывает «20.10.2026», в поле уходит ISO), лента с догрузкой (`read_page` предупреждает, что читает только загруженное).
...
`gate_eval.py` проверяет политику на 9 ситуациях — удаление по задаче и без, оплата, отклик, иконка в корзине, системное окно; корзина, фильтры, чекбокс — не high. Каждая ситуация дважды, на устойчивость. Меньше цента за прогон.
...
| механика агента | `pytest` — 84 теста, ~1 минута: заглушки вместо модели, Chrome без окна на синтетических страницах (модалки, календарь, прозрачные чекбоксы, поле с автовозвратом, скрытые фреймы), сквозной прогон цикла | бесплатно |
| политика безопасности | `gate_eval.py` — 9 ситуаций × 2 | < 1 цента |
...
- **Windows и Linux** — код и файлы запуска кроссплатформенные, живые прогоны были только на macOS.
...
- **Письма.** Писатель не добавляет того, чего нет ни в резюме, ни в вакансии, но перенос проекта на другого работодателя автоматически не ловится — письма стоит перечитывать.
...
- **Даты в календаре.** Ячейка дня подписана только числом, месяц — текстом выше; однажды агент промахнулся на десяток дней. Кандидат: подписывать ячейку заголовком её группы.
